...
export default async function DepartmentTasksGanttPage({
  params,
}: {
  ...
  return (
    <div className="mx-auto max-w-[1680px] px-[var(--pad-x)] pb-14 pt-6">
      <p className="mb-6 max-w-2xl text-sm font-medium leading-relaxed text-app-text">
        Zeitleiste nur für <strong>{dept.name}</strong> — dieselben URL-Parameter wie in der Tabelle
        dieser Abteilung.
      </p>
      {error ? (
      ...
      <Suspense fallback={<div className="text-sm text-app-muted">Gantt wird geladen…</div>}>
        <TaskListFilteredGantt initialRows={operational} departmentId={dept.id} />
      </Suspense>
    </div>
  );
}
